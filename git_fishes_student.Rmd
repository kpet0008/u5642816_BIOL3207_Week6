...
# The code block will not be visible in the rendered doccument
```
...
means_temp <- data_clean %>% group_by(species) %>%  mutate(meansl = mean(sl)) %>%  mutate(meanact = mean(activity))

means <- data.frame(unique(means$species), unique(means$meansl), unique(means$meanact))


#means <- data_clean %>% group_by(species) %>%  mutate(meansl = mean(sl)) %>%  mutate(meanact = mean(activity))

#head(means_sl)

#means_new <- means %>% expand(species, meansl, meanact)

#(unique(means$species), unique(means$meansl), unique(means$meanact))

#means_new <- data.frame(unique(means$species), unique(means$meansl), unique(means$meanact))
...
ggplot(data_clean, aes(activity, sl, color=treatment)) + geom_point() + facet_wrap(~species)

```
```{r eval=F}
#ggsave('./output/figures', width = 6, height = 4)
```
