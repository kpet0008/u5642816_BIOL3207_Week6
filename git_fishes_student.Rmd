...
## Is this even what Im being asked to do? Is this "permutations"? check laterS
```{r}
temp_data <- data_clean %>% mutate(treatment = sample(treatment, length(treatment), replace = F)) %>% filter(species == "acantho")

t.test(temp_data$sl[temp_data$treatment == "CO2"],temp_data$sl[temp_data$treatment == "control"],var.equal=T)

```

The results were never going to differ, because as demonstrated earlier, they arent significantly different from random to begin with.
