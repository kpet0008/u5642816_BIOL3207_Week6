...
data_clean <- data_cleaning[-c(1,2,9)]
```
...
```{r}
## There shouldnt be any figures in that folder on desktop. However I didnt set up the .gitignore file correctly the first time and so ended up with an image in there anyway, and it seems like its not possible to remove it. Additional images are no longer uploaded to that folder now that I've fixed it.
```
...
Mean activity for `r means.control$Species[1]` in the control group was `r x=means.control$Mean.Activity[1]; x` (s / min) compared to the OA treatment group, which was `r means.treatment$Mean.Activity[1]` (s / min). The difference between control and OA treatment means was `r means.control$Mean.Activity[1]-means.treatment$Mean.Activity[1]` (s / min) (95% CI: `r "code for lower CI"` to `r "code for upper CI"`). 
...
The null hypothesis being tested would be that there is no difference in activity level between the control group and the treatment group. With the statistical evidence calculated [blah blah blah code here], this null hypothesis cannot be rejected.
...
length(which(samp_dist$extreme)) / length(which(!samp_dist$extreme))

```
```{r}
#A reminder of the real data's t-statistic, for easy comparison
#filter data by ith level of the species factor
temp_data <- filter(data_clean, species == levels(data_clean$species)[1])
#run stat test with subset
t.test(temp_data$activity[temp_data$treatment == "CO2"],temp_data$activity[temp_data$treatment == "control"],var.equal=T)
```
The p-value from the permutation analysis is higher than from the t-test. However, both are too high to reject the null hypothesis regardless/
